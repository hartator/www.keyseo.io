...
(function(){
  ...
  var HalfViz = function(elt){
    ...
    sys = arbor.ParticleSystem(2600, 64, 0.5)
    ...
    var that = {
      ...
      resize:function(){
        var sizeCorrect = 150
        if ($(window).width() < 871) sizeCorrect = 0  
        if( window.innerHeight == screen.height) sizeCorrect = 0  
        var w = $(window).width() - sizeCorrect
        var x = w - _ed.width()
        that.updateLayout(x)
        sys.renderer.redraw()
      },
  ...
  $(document).ready(function(){
    ...
    var real_time_loop = function real_time_loop() {
        $.get( "/real_time/get_data", function( data ) {
          window.src += data;
          window.mcp.updateGraph();
          $("#dot").pulse({color: 'red'}, {duration: 300});
          setTimeout(real_time_loop, 2000)
          if (window.src != "") $('#message-canvas').hide()
        });
    }
